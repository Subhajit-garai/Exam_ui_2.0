...
export const QuizLeaderboard = ({ results, quizId }: QuizLeaderboardProps) => {
    ...
    const currentData = useMemo(() => {
        ...
        if (sourceData && sourceData.length > 0) {
            return sourceData.map((item: any, index: number): LeaderboardEntry => ({
    ...
    return (
        <LeaderboardComponent
            title="Leaderboard"
            icon={<BrainCircuit className="text-indigo-600 fill-indigo-200" size={28} />}
            timeframe={timeframe}
            ...
            data={currentData}
            renderItem={(item) => (
                <div key={(item.name + item.score).toString()} className="flex items-center justify-between py-1">

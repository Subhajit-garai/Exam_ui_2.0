...
interface QuizLeaderboardProps {
    results?: any[]; // Allow external results to be passed in
    ...
    const currentData = useMemo(() => {
        ...
        if (sourceData && sourceData.length > 0) {
            return sourceData.map((item, index) => ({
                user: {
                    id: item.userId || item.user?.id || `user-${index}`,
                    name: item.userName || item.name || item.user?.name || "Participant",
                    avatar: (item.userName || item.name || item.user?.name || "P").substring(0, 2).toUpperCase()
                },
                ...
                {isLoading ? (
                    ...
                    currentData.map((item) => (
                        <div key={item.user.id} className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{item.score} Points</span>
                        </div>
                    ))
                )}
            </div>
